...
use itybity::{BitIterable, ToBits};
use mpz_core::Block;
...
impl Receiver {
    ...
    /// ## ⚠️ Warning ⚠️
    ///
    /// If the receiver is committed, the receiver's RNG seed must be unbiased
    /// such as generated by a secure coin toss protocol with the sender.
    ///
...
impl Receiver<state::Setup> {
    /// Computes the decryption keys, returning the Receiver's payload to be
    /// sent to the Sender.
    ///
    /// # Arguments
    ///
    ...
    /// Receives the encrypted payload from the Sender, returning the plaintext
